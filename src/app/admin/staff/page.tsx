      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white  shadow rounded-xl p-6 border border-gray-200 ">
            <h2 className="text-xl font-semibold text-gray-900  mb-4 flex items-center"><UserPlus className="mr-2" size={20}/> Manage Staff</h2>
          ...
          <div className="bg-white  shadow rounded-xl p-6 border border-gray-200 ">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-gray-900  flex items-center"><Clock className="mr-2" size={20}/> Time Tracking</h2>
            ...
            <table className="min-w-full divide-y divide-gray-200  mt-4">
              ...
              <tbody className="divide-y divide-gray-200 ">
                {Object.values(hoursMap).map((data) => {
                  ...
                  return (
                    <tr key={data.name}>
                      <td className="py-3 text-sm font-medium text-gray-900 ">{data.name}</td>
                      <td className="py-3 text-sm text-right text-gray-700  font-mono">{hours} hrs</td>
                    </tr>
                  )
                })}
              </tbody>
        ...
        <div className="space-y-8">
          <div className="bg-white  shadow rounded-xl p-6 border border-gray-200 ">
            <h2 className="text-xl font-semibold text-gray-900  mb-4 flex items-center"><CheckCircle2 className="mr-2" size={20}/> Daily Tasks Overview</h2>
            ...
            <form action={async (formData) => {
              ...
              <label className="text-sm font-medium text-gray-700 ">Add New Task</label>
              <input type="date" name="task_date" defaultValue={selectedDate} required className="rounded-md border border-gray-300  px-3 py-2 bg-white  text-gray-900  sm:text-sm" />
              <div className="flex gap-2">
                <input type="text" name="description" placeholder="Task description (e.g. Muck out stalls 1-5)" required className="flex-1 rounded-md border border-gray-300  px-3 py-2 bg-white  text-gray-900  sm:text-sm" />
            ...
            <h3 className="font-medium text-gray-900  mb-3 border-b border-gray-200  pb-2">Tasks for {selectedDate}</h3>
            <ul className="space-y-3">
              {tasks.length === 0 ? (
                <p className="text-sm text-gray-500">No tasks assigned for this date.</p>
              ) : (
                tasks.map((task: any) => (
                  <li key={task.id} className="flex justify-between items-center p-3 bg-white  rounded-lg border border-gray-100 ">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-sm ${task.is_completed ? 'line-through text-gray-400' : 'text-gray-900  font-medium'}`}>
                        {task.description}
                      </span>
                    </div>
