...
import { getTeamMember, updateTeamMember } from '@/app/actions/team'
...
export default function EditTeamMemberPage(props: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadMember() {
      try {
        ...
        setError(err.message || 'Failed to load team member details')
      } finally {
        setIsLoading(false)
      }
    }
  ...
  if (!member && !isLoading) {
    return <div className="text-center p-12 text-red-500">Team member not found or error loading data.</div>
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-serif font-bold text-gray-900 ">Edit Team Member</h1>
      </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              <div className="col-span-2 sm:col-span-1">
                ...
                <input required defaultValue={member.name} type="text" name="name" id="name" className="mt-1 block w-full rounded-md border border-gray-300  px-3 py-2 bg-white  text-gray-900  focus:border-primary focus:ring-primary sm:text-sm" />
              </div>

              <div className="col-span-2 sm:col-span-1">
                <label htmlFor="role" className="block text-sm font-medium text-gray-700 ">Role / Title *</label>
                <input required defaultValue={member.role} type="text" name="role" id="role" className="mt-1 block w-full rounded-md border border-gray-300  px-3 py-2 bg-white  text-gray-900  focus:border-primary focus:ring-primary sm:text-sm" />
              </div>
            </div>

            <div>
              <label htmlFor="sort_order" className="block text-sm font-medium text-gray-700 ">Sort Order</label>
              <input type="number" defaultValue={member.sort_order} name="sort_order" id="sort_order" className="mt-1 block w-full rounded-md border border-gray-300  px-3 py-2 bg-white  text-gray-900  focus:border-primary focus:ring-primary sm:text-sm" />
            ...
            <div>
              ...
              <textarea name="bio" defaultValue={member.bio || ''} id="bio" rows={4} className="mt-1 block w-full rounded-md border border-gray-300  px-3 py-2 bg-white  text-gray-900  focus:border-primary focus:ring-primary sm:text-sm" />
            </div>
