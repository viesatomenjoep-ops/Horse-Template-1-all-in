...
import { getHorseResults, addHorseResult, deleteHorseResult } from '@/app/actions/horse_results'
import { Plus, Trash2, Video, Trophy } from 'lucide-react'

export default function HorseResultsManager({ horseId }: { horseId: string }) {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Weet je zeker dat je dit resultaat wilt verwijderen?")) {
      try {
        await deleteHorseResult(id, horseId)
        await fetchResults()
      } catch (err) {
        console.error(err)
